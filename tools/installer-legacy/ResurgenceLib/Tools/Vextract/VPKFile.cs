...
namespace ResurgenceLib.Tools.Vextract
{
    /// <summary>
    ...
    public sealed class VPKFile
    {
        ...
        /// </summary>
        private VPKSupport.DirectoryInfo DirectoryInfo;

        /// <summary>
        /// The list of directory entries.
        /// </summary>
        private List<VPKSupport.DirectoryEntry> DirectoryEntries;
        ...
        public VPKFile(string Path)
        {
            ...
            DirectoryInfo = new VPKSupport.DirectoryInfo(VPKReader);
            DirectoryEntries = new List<VPKSupport.DirectoryEntry>(DirectoryInfo.FileCount);
            ...
            for (int i = 0; i < DirectoryInfo.FileCount; i++)
                DirectoryEntries.Add(new VPKSupport.DirectoryEntry(VPKReader));
        ...
        /// </summary>
        /// <returns></returns>
        internal VPKSupport.DirectoryEntry[] GetEntries()
        {
        ...
        {
            int fileCount = 0;
            List<VPKSupport.DirectoryEntry> entries = findMatches(pattern);
            ...
            foreach (VPKSupport.DirectoryEntry currentEntry in entries)
            {
        ...
        /// <returns></returns>
        private byte[] readContents(VPKSupport.DirectoryEntry entry, BinaryReader VPKReader)
        {
        ...
        /// </param>
        /// <returns></returns>
        private List<VPKSupport.DirectoryEntry> findMatches(string pattern)
        {
            List<VPKSupport.DirectoryEntry> entries = new List<VPKSupport.DirectoryEntry>();

            foreach (VPKSupport.DirectoryEntry entry in DirectoryEntries)
